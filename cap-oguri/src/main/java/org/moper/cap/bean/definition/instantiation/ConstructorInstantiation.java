 ...
 * 基于构造函数的实例化策略 </br>
 *
 * @param constructor 要实例化的类，不能为 null
...
public record ConstructorInstantiation(
     ...
     * @return 构造函数实例化策略实例
     */
    public static ConstructorInstantiation of(Constructor<?> constructor) {
        return new ConstructorInstantiation(constructor, new Class<?>[0]);
    }
     ...
     * @return 构造函数实例化策略实例
     */
    public static ConstructorInstantiation of(Constructor<?> constructor, Class<?>... argTypes) {
        return new ConstructorInstantiation(constructor, argTypes);
    }
}
